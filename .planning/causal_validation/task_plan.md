...
- **Status:** Stage A has 26 promoted roots: 16 closed roots from v32, the two-state v31 open hard pair, one v34 open root, and seven v35 roots. All newly required on-table roots are complete; their replicated A/B cabinet basins remain layout-sensitive after registration (`1/46` in A versus `4–5/46` in B). Both prospective transverse episode-474 attempts were imported exactly with zero re-execution. v35 stopped before proposal execution on its first grasped layout-A root because contact continuity exceeded the frozen gate. Construction-only diagnostics reject extra settling and slower motion as fixes. The next gate is a bounded grasp-topology diagnostic followed by one predeclared six-root successor shard; cross-aperture/source-revision and cross-oracle-mode effects remain non-estimable, and Stages B/C are separate simulation-scope decisions.
