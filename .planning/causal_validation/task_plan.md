...
- **Status:** Stage A has 19 unique promoted roots: 16 closed roots from v32, the two-state v31 open hard pair, and one v34 open root. The v34 layout-B `18/36` drawer and `0/46` cabinet ledgers are frozen. The bowl-relative registration discovered on that near-support pair has now passed a prospective one-proposal gate on both untouched transverse layout roots (cabinet episode 474 at source frames `83/79`). The next gate is a fresh 13-candidate additive completion shard that imports those exact attempts and the completed v34 drawer ledger by hash, then simulates only missing proposal evidence. Cross-aperture/source-revision and cross-oracle-mode effects are non-estimable; Stages B/C remain separate simulation-scope decisions.
