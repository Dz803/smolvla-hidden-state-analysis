...
- **Status:** Stage A has 25 promoted roots: 16 closed roots from v32, the two-state v31 open hard pair, one v34 open root, and six v35 roots. The repaired v35 layout-B root preserves its v34 drawer/negative-cabinet ledgers and changes cabinet coverage from `0/46` world-anchor successes to `5/46` registered successes on the identical normalized state. Both prospective transverse episode-474 attempts were imported exactly with zero re-execution. Candidate 6 exposed and survived a post-bank registered-mode validator omission through a hash-bound compatibility recovery with zero proposal reruns. Expansion continues from v35 candidate 7/13. Cross-aperture/source-revision and cross-oracle-mode effects remain non-estimable, and Stages B/C are separate simulation-scope decisions.
