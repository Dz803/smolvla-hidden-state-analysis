...
### 2026-07-31 — v35 registered held-root generalization gate

- Froze a translation-only v35 rule using layout A as the declared reference: retain each action-derived EEF-to-bowl anchor offset, translate it to the target layout's bowl landmark, and apply no orientation transform. The normalized target landmark is checked within `1e-9 m`.
- Prospective raw smoke `registered_smoke_20260731T073347Z` runs cabinet episode 474 once on the previously untouched transverse-low-support layout-A/B roots. Both certified roots pass: layout A reaches the goal at source frame 83 and layout B at frame 79, with valid bridges, zero bowl drift, preserved drawer aperture, and no wrong-goal or early-terminal event.
- The result rejects repair-to-one-root overfitting but remains one proposal, one physical family, and one LIBERO task. Exact attempt payloads are retained for hash-bound import into the completion shard and must not be rerun. Compact evidence is `reports/phase3b_stage_a/registered_smoke_20260731T073347Z/`.

### 2026-07-31 — v35 full-bank alignment effect and additive promotion
...
### 2026-07-31 — v36 grasp-topology repair and six-root construction smoke

- A diagnostic-only continuity observer shows that the failed v35 acquisition is not ordinary solver flicker: the bowl changes its end-effector-relative pose by `20.94 mm` during the first lift, ends re-seated by `20.41 mm`, and then transports with zero further contact dropout. Longer settling and slower motion had already failed, so the production continuity thresholds remain unchanged.
- A prospective phase-separated diagnostic opens the drawer with episode 382, then uses only the bowl-registered pre-grasp continuation from cabinet episode 474 until three stable grasp ticks. Both layouts pass with zero bridge bowl drift, no goal/terminal event, final possession, and at most `1.33 mm` safe-lift relative drift.
- v36 assigns exactly the six untouched open/grasped roots, imports no prior proposal attempts, and normalizes them at timestep 560. The value is measured rather than guessed: layout B reaches timestep 548 before padding, and the contract reserves ten stability ticks plus at least one explicit padding action. The resulting worst-case native oracle horizon is `2,064 < 2,200`.
- Final policy-free smoke `local/phase3b_stage_a/construction_gates/phase3b_stage_a_construction_gate_20260731T094517Z` passes all six roots, all available pair-geometry audits, support and root gates, and fresh repeated-action certificates. It produces six unique state hashes under support bank `456f9c96…7f39` and executes zero goal proposals. The next authorised spend is the fresh 492-attempt v36 proposal shard; no completed v31–v35 attempt will be rerun.

### 2026-07-31 — v36 normalized-landmark stop and v37 execution binding

- v36 completion run `phase3b_stage_a_completion_v36_20260731T104403Z` reconstructed its first root at the exact construction-gate hash, then failed before proposal index 0 because post-set-down bowl position was compared to the nominal layout landmark at the `1e-9 m` layout-registration tolerance. Its only checkpoint contains zero results, so no rollout was completed or promoted.
