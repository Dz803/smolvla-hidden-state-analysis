...
Phase 3b Stage A is complete. The [`verified 32-state consolidation`](reports/phase3b_stage_a/phase3b-stage-a-consolidated-v4/) contains 32 distinct roots, 16 physical support pairs, 64/64 physically feasible state-goal cells, and all 2,624 proposal attempts. One cabinet cell is physically recoverable despite a complete registered `0/46` bank; the [`compact competence–compatibility report`](reports/phase3b_stage_a/competence_compatibility_gap_v1/) keeps those evidence classes separate. This is not a SmolVLA capability or hidden-state result. The [`cross-model methodology and literature boundary`](docs/competence_compatibility_methodology.md) define the next state-matched `F_C/V/Q/L` and self-specificity tests. Stage B and external checkpoint downloads remain separately gated.
